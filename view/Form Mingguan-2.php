<?php
  session_start();
  require('../controller/connect.php'); 
  include('../controller/loginController.php');

  if($_SESSION['jabatan'] == '3') {
    $jabatans = "Dashboard Penghuni.php";
  } elseif ($_SESSION['jabatan'] == '2'){
    $jabatans = "Dashboard ketua.php";
  }

?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Document</title>
    <link rel="stylesheet" type="text/css" href="CSS/Form Mingguan2.css">
</head>
<body>
<form action="../controller/formmingguanpage2.php" method="post" class="form-mingguan">
  <div class="rectangle-189"></div>
  <div class="rectangle-190"></div>

  <a href="<?= $jabatans ?>">
  <svg
    class="vector"
    width="16"
    height="16"
    viewBox="0 0 16 16"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path
      d="M8 16L0 8L8 0L9.425 1.4L3.825 7H16V9H3.825L9.425 14.6L8 16Z"
      fill="white"
    />
  </svg></a>

  <div class="form-mingguan2">Form Mingguan</div>
  <div class="rectangle-1480"></div>
  <input name="dhuha" class="rectangle-23">
  <div class="rectangle-1481"></div>
  <input name="infaq" class="rectangle-1482">
  <div class="rectangle-1483"></div>
  <input name="wirid" class="rectangle-1484">
  <div class="rectangle-14802"></div>
  <input name="olahraga" class="rectangle-232">

  <div class="catat-aktivitas-olahraga">Catat Aktivitas Olahraga</div>
  <div class="catat-aktivitas-wirid-pagi-dan-petang">
    Catat Aktivitas wirid pagi dan petang
  </div>
  <div class="catat-aktivitas-infaq">Catat Aktivitas Infaq</div>
  <div class="catat-aktivitas-dhuha">Catat Aktivitas Dhuha</div>
  <input type="submit" value="Berikutnya" class="rectangle-1485">
  
</form>

</body>
</html>